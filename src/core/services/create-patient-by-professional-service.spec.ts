...
describe('Create Patient By Professional Service', () => {
  beforeEach(() => {
    inMemoryPatientRepository = new InMemoryPatientRepository();
    inMemoryProfessionalsRepository = new InMemoryProfessionalsRepository();
    inMemoryUserRepository = new InMemoryUserRepository();

    sut = new CreatePatientByProfessionalService(
  ...
  it('should create a patient for an existing professional', async () => {
    ...
    const result = await sut.handle({
      professionalId: user.id.toString(),
      name: 'Jane Doe',
    ...
    expect(inMemoryPatientRepository.items[0].note).toEqual('note');
  });

  it('should not create a patient when user does not exist', async () => {
    const result = await sut.handle({
      professionalId: 'user-02',
      name: 'Jane Doe',
      birthDate: '1990-01-01',
      phone: '987654321',
      note: 'note',
